...
impl KmerLookup for crate::counter::KmerCount {
    ...
    fn kmer_len(&self) -> usize { self.kmer_len() }
    fn find(&self, kmer: &Kmer) -> usize { self.find(kmer) }
    fn get_count(&self, index: usize) -> u64 { self.get_count(index) }
    fn update_count(&mut self, count: u64, index: usize) { self.update_count(count, index) }
    fn get_kmer_count(&self, index: usize) -> (Kmer, u64) { self.get_kmer_count(index) }
    fn build_hash_index(&mut self) { self.build_hash_index() }
}
